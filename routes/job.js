...
router.get("/jobapplication", function (req, res) {
  companysignup.findById({ _id: req.user._id }, (err, result) => {
    if (err) console.log(err);
      ...
      Job.find({ CompanyName: result.CompanyName })
        .populate("jobresume")
        .exec((err, ok) => {
          if (err) console.log(err);
          else {
...
router.post(
  "/jobapplication/:jobid/jobseeker/:jobseekid/delete",
  (req, res) => {
      ...
      Job.updateMany(
        { _id: req.params.id },
        {
          $set: {
            ...
            Welfare: Welfare,
            Province: Province,
            Qualificationsofjobapplicants: Qualificationsofjobapplicants,
            Enddate: Enddate,
            Contact: Contact,
            JobDescription: JobDescription,
            Howtogocompany: Howtogocompany,
            Address: Address,
            image: ok.image,
  ...
  Job.findById({ _id: req.params.id }, (err, job) => {
    if (err) console.log(err);
      ...
      jobseekersignup.findById({ _id: req.user._id }, (err, user) => {
        if (err) res.redirect("/jobseeker/login");
        else {
      ...
      Job.create(
        {
          ...
          Howtogocompany: req.body.Howtogocompany,
          ...
          Editdate: Date.now(),
          Qualificationsofjobapplicants: req.body.Qualificationsofjobapplicants,
          image: job.image,
        },
        function (err, addjob) {
          if (err) {
